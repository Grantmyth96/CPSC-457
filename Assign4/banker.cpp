...
//////////// CHECK PREREQUISITES//////////////////
        for(int i=0; i<numResources; i++) {
          ...
          //Set new allocation and available values
          available[i] -= req[i];
...
/////////////// CHECK PREREQUISITES//////////////////
        ...
        while(t < (numProc*10)) {
          for(int pr=0; pr<numProc; pr++) {
            for(int i=0; i<numProc; i++) {
              if(pr==finish[i] && pr<numProc) pr++;
              if(pr>=numProc) {
                
                if(exit == numProc) return true;
              }
            }
            
            for(int i=0; i<numResources; i++) {
              
              if((max[pr][i]-allocation[pr][i]) <= available[i]) {
                count++;
              }
            }
            ...
            if(count == numResources) {
              for(int i=0; i<numResources; i++) {
                available[i] += allocation[pr][i];
              }
              if(pr==numProc-1) sequenceOrReason += "P"+to_string(pr);
              else sequenceOrReason += "P"+to_string(pr)+", ";
